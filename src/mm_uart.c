...
int mm_uart1_init(void)
{
    ...
    stat = uart_driver_install(MM_UART_1, MM_UART_BUF_SIZE_256, MM_UART_BUF_SIZE_256, 0, NULL, 0);
    if( stat != OK)
    {
    ...
    uart_param_config(MM_UART_1, &mm_uart1_config);
    if( stat != OK)
    {
    ...
    uart_set_pin(MM_UART_1, MM_UART_1_TX_PIN, MM_UART_1_RX_PIN, UART_PIN_NO_CHANGE, UART_PIN_NO_CHANGE);
    if( stat != OK)
    {
